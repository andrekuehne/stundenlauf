...
| 2026-04-13 | Cursor Cloud + npm reproducibility for TS port | Documented Node/npm workflow in root `AGENTS.md`; `.nvmrc` + `engines.node`; fixed ESLint TypeScript config loader (`jiti`), Node types for `tsc -b`, ESM-safe Vite/Vitest path aliases so `npm ci` and `npm run build` succeed on Linux agents |

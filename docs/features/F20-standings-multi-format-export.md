...
**Laufübersicht presentation (v2):** The PDF starts with a **cover page** (large blue **year** only, then a centered **Hinweis** paragraph with underlined label and default Pokal-/Gesamtwertung copy), then a page break before the first category table—unless `pdf.laufuebersicht_show_cover` is `false`. Override the notice body with `pdf.laufuebersicht_notice` (non-empty string replaces the default). When `pdf.title` is empty, each section title is **`{exportIndex}. {duration} - {division}`** (e.g. `1. Halbstundenlauf - Frauen`); a non-empty `pdf.title` is reused for every section (same as flat exports). Table chrome: pale **green** header background; **red** text on the merged **n. Lauf** / **Gesamt** labels (row 1); **double** horizontal rule under the header; **thick** vertical after **Verein**; **dashed** verticals between each run’s Str. and Wertung columns (and between Gesamt Str./Pkt.); **double** vertical between the last race block and **Gesamt**; places 1–3 get a **light blue** row tint instead of yellow.

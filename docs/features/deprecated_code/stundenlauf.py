...
def split_race_data(file_path):
    ...
    for i in range(0, len(indices), 3):
        ...
        maenner = process_section(
            df.iloc[maenner_index + 1 : indices[i + 3] if i + 3 < len(indices) else None].reset_index(drop=True)
        )
...
def merge_race_dataframes(df_list):
    ...
    pivoted = combined_df.pivot(index=["Name", "Jahrg.", "Verein"], columns="race_num", values=["Distanz", "Punkte"])
...
def merge_similar_names_old(df):
    def parse_name(name):
        ...
        return {"first": parsed.first, "middle": parsed.middle, "last": parsed.last, "full": str(parsed)}
        ...
        for i, name1 in enumerate(names):
            for name2 in names[i + 1 :]:
                ...
                if similarity > 80:  # Threshold can be adjusted
                    ...
                    if not any(
                        (rows1[col].notna() & rows2[col].notna()).any()
                        for col in df.columns
                        if col not in ["Name", "Jahrg.", "Verein"]
                    ):
...
def calculate_totals(df):
    ...
    df_sorted = df.sort_values(by=["Punkte gesamt", "Distanz gesamt"], ascending=[False, False]).reset_index(drop=True)
    ...
    merged_columns = [col for pair in zip(distanz_columns, punkte_columns, strict=False) for col in pair]
...
def output_from_template_einzellauf(
    template_file: str, df: pd.DataFrame, new_text_A1: str, output_file: str = "updated_template.xlsx"
) -> str:
    """
    ...
    thick_border = Border(
        left=Side(style="thick"), right=Side(style="thick"), top=Side(style="thick"), bottom=Side(style="thick")
    )
    ...
    for i, row_data in df.iterrows():
        ...
        if (
            formula_O_template_str
            and isinstance(formula_O_template_str, str)
            and formula_O_template_str.startswith("=")
        ):
        ...
        if (
            formula_P_template_str
            and isinstance(formula_P_template_str, str)
            and formula_P_template_str.startswith("=")
        ):
...
def merge_excel_files(input_files, output_file, output_sheet_name):
    ...
    for file_idx, file_path in enumerate(input_files):
                ...
                if input_cell.data_type == "f":
                    ...
                    if formula and isinstance(formula, str) and formula.startswith("="):
                        ...
                        try:
                            ...
                            print(
                                f"    Warning: Failed to translate formula '{formula}' from {origin_address} (file {file_path}) to {destination_address}. Copying original. Error: {e}"
                            )
        ...
        file_row_mapping.append({"output_start_row": start_row_for_this_file, "input_ws": input_ws})

        # Add separator row
        if file_idx < len(input_files) - 1 and max_row_in_input > 0:
            current_row_in_output += 1
    ...
    for mapping in file_row_mapping:
        ...
        for merged_range in input_ws_ref.merged_cells.ranges:
            ...
            try:
                output_ws.merge_cells(
                    start_row=new_min_row, start_column=min_col, end_row=new_max_row, end_column=max_col
                )
            except ValueError as e:
                print(
                    f"    Warning: Could not merge range {get_column_letter(min_col)}{new_min_row}:{get_column_letter(max_col)}{new_max_row}. Error: {e}"
                )
            except Exception as e:
                print(
                    f"    Warning: Unexpected error merging range {get_column_letter(min_col)}{new_min_row}:{get_column_letter(max_col)}{new_max_row}. Error: {e}"
                )

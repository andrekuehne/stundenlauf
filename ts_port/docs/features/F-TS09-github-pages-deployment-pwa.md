    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 22
          cache: npm
          cache-dependency-path: ts_port/package-lock.json
      - run: npm ci
      - run: npm run lint
...
| Decision | Rationale |
|---|---|
| `paths` filter on `ts_port/**` | Avoids deploying on Python-only changes. |
...
The existing `windows-build.yml` fires on `v*` tags and builds the Python desktop app. The new `ts-deploy.yml` fires on pushes to `main` (filtered to `ts_port/**` paths). The two workflows are independent:

| Aspect | Python (`windows-build.yml`) | TS Port (`ts-deploy.yml`) |
|---|---|---|
| Trigger | Push of `v*` tag | Push to `main` (ts_port paths) |
...
1. **Initialize Vite project** — if not already done, scaffold the `ts_port/` directory with `npm create vite@latest` (React + TypeScript template). Configure `tsconfig.json` strict mode, ESLint, Prettier.
...
9. **Create GitHub Actions workflow** — `.github/workflows/ts-deploy.yml` with quality (lint + typecheck + test), build, and deploy jobs. Scoped to `ts_port/**` paths.
...
- **Integration: GitHub Actions**
  - A push to `main` with `ts_port/` changes triggers the workflow.
